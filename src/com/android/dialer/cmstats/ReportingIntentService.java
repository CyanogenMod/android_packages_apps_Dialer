...
public class ReportingIntentService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        if (!bundle.isEmpty()) {
            ...
            builder.setCategory(bundle.getString(Fields.EVENT_CATEGORY));
            builder.setAction(bundle.getString(Fields.EVENT_ACTION));
            if (bundle.containsKey(Fields.EVENT_LABEL)) {
                builder.setLabel(bundle.getString(Fields.EVENT_LABEL));
            }
            if (bundle.containsKey(Fields.EVENT_VALUE)) {
                builder.setValue(value);
            }
            String event = builder.build();
            mTracker.send(event);
        }
    }
}
